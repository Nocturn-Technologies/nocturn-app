...
/**
 * TEMPORARY admin endpoint to resend a ticket confirmation email.
 * Protected by a secret token. Remove after use.
 */
export async function POST(request: NextRequest) {
  const { token, ticketId } = await request.json();

  // Simple auth — temporary hardcoded secret (route will be removed after use)
