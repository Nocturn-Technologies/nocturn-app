...
export async function getEventFinancials(eventId: string): Promise<{ error: string | null; data: EventFinancials | null }> {
  try {
    ...
    if (tiersRes.error || ticketsRes.error || expensesRes.error || artistsRes.error) {
      console.error("[getEventFinancials]", tiersRes.error || ticketsRes.error || expensesRes.error || artistsRes.error);
