...
export async function bulkTagContacts(
  ...
  // Rate limit: 10 bulk ops per minute
  const rl = await rateLimitStrict(`bulk-tag:${auth.userId}`, 10, 60_000);
  if (!rl.success) return { error: "Rate limit exceeded. Try again in a minute.", updated: 0 };

  if (contactIds.length === 0) return { error: "No contacts provided", updated: 0 };
  if (contactIds.length > 100) return { error: "Maximum 100 contacts per bulk operation", updated: 0 };
  if (tags.length === 0) return { error: "No tags provided", updated: 0 };
  ...
  if (!existing || existing.length === 0) {
    return { error: "No matching contacts found", updated: 0 };
  }

  let updatedCount = 0;

  // Update each contact's tags (merge, don't replace)
  for (const row of existing as { id: string; tags: string[] }[]) {
...
export async function exportContactsCSV(
  ...
  const rl = await rateLimitStrict(`export-contacts:${auth.userId}`, 5, 60_000);
  if (!rl.success) return { error: "Rate limit exceeded. Try again in a minute.", csv: "" };

  const admin = createAdminClient();

  // Build query (similar to getContacts but no pagination — up to 10,000)
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  let query = (admin.from("contacts") as any)
    .select("*")
    .eq("collective_id", collectiveId)
  ...
  if (error) return { error: `Export failed: ${error.message}`, csv: "" };
  ...
  const csvRows = contacts.map((c) => [
    ...
    csvSafe(c.phone ?? ""),
    csvSafe(c.instagram ?? ""),
    csvSafe(c.contactType),
    csvSafe(c.role ?? ""),
    csvSafe(c.source),
    csvSafe((c.tags ?? []).join(", ")),
    csvSafe(c.totalEvents.toString()),
    csvSafe(`$${c.totalSpend.toFixed(2)}`),
    csvSafe(c.firstSeenAt ? new Date(c.firstSeenAt).toLocaleDateString("en-US") : ""),
    csvSafe(c.lastSeenAt ? new Date(c.lastSeenAt).toLocaleDateString("en-US") : ""),
    csvSafe(c.notes ?? ""),
  ]);
